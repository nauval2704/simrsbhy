...
var ze = (() => {
  class t {
    ...
    renderTable() {
      ...
      if (this.loading) {
        root.innerHTML =
          '<div class="container-fluid mt-3"><div class="d-flex justify-content-center mt-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Memuat...</span></div></div></div>';
        return;
      }
      if (this.error) {
        root.innerHTML =
      ...
      if (this.patients.length === 0) {
        rows =
          '<tr><td colspan="12" class="text-center text-muted py-3 fw-bold">Tidak ada antrean laboratorium dari IGD saat ini.</td></tr>';
      } else {
          ...
          let btn = `
            <div class="dropdown" style="position: relative;">
                ...
                <div class="dropdown-menu shadow" style="position: absolute; z-index: 1000;">
                    <button class="dropdown-item" onclick="alert('Route to Input Laboratorium for: ${p.noCheckin}')"><i class="bi bi-chevron-right me-2"></i> Input Laboratorium</button>
                </div>
            </div>
          `;
          let encounterBadge =
      ...
      root.innerHTML =
        ...
        '<div class="card mb-2">' +
        '<div class="card-header bg-warning fw-bold text-dark">LABORATORIUM</div>' +
        '<div class="card-body">' +
